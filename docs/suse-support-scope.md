...
|-----------|-----------|-----------|----------|
| **(a) Custom CRDs should be namespace-scoped** to avoid cluster-wide configuration pollution and respect Kubernetes admission policy intent. | Prevent custom CRDs from silently consuming cluster-wide slots or interfering with future GA features via name collisions. | **DEVIATION — by design, not within our control.** | KubeOVN's Vpc, Subnet, ProviderNetwork, Vlan, VpcNatGateway, IptablesEIP, IptablesSnatRule are cluster-scoped in upstream KubeOVN; there is **no namespaced variant** to switch to. Not a choice of ours. We mitigate with ownership labels. See deviation #1. |
...
### 1. Cluster-Scoped kubeovn.io CRDs

**Within our control? No — by upstream design.** Upstream KubeOVN ships no namespaced variant of these CRDs; we cannot change their scope.
