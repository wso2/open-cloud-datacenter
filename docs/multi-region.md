...
## 13.2 Routing a resource family through the agent (the capability registry)

Which resources the agent is allowed to manage, and which permissions its ServiceAccount gets, are declared in one place: the capability registry (`internal/providers/clusteraccess/capabilities.go`). Each entry names a resource family and lists two things — the operations dc-api may route to the agent (`RouteVerbs`), and the Kubernetes verbs the agent's ServiceAccount needs (`AgentVerbs`). From those declarations, three things are generated automatically: the routing allow-set dc-api consults, the resource-to-kind mapping, and the agent's RBAC ClusterRole (`flux/platform/dc-agent/base/rbac.yaml`, regenerated by `make gen-agent-rbac` and guarded by a drift-check test). Onboarding a family is a declaration plus wiring that family's provider to call the cluster-access seam — not edits spread across several files.

The decision of whether to route is made **per resource family**, not globally: `agentDecision` looks up the specific resource's allowed verbs (`RoutableVerbs(gvr)`), so one family routing a verb never causes another family to route it. This is what lets more than one family be onboarded safely.

Onboarded so far:

- **Virtual machines** (KubeVirt) — create, read, delete.
- **KubeOVN network resources** — the VPC, subnet, and network-attachment-definition create/read/delete that the network reconciler drives end to end. A VNet or subnet is created and deleted entirely through the agent, so a single resource's lifecycle never splits across two credentials.

Deliberately still on the direct client (a later change):

- The **plumbing layered on top** of those resources: the per-VPC NAT gateway, the per-VPC DNS, namespace and quota setup, and the best-effort stuck-finalizer cleanup. Routing these needs verbs (pods, IPs, configmaps, patch-on-parent) that the network families don't declare, so they are better moved as their own coherent change.
- Resources reached over a **REST API** rather than the cluster's Kubernetes API — Rancher clusters and the OpenBao key vault. The agent only performs Kubernetes operations on its own cluster, so these would need a different kind of agent capability and are out of scope for now.
